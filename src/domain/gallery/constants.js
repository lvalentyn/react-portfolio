export const getGalleryConfig = () => {
  ...
  const dragSensitivity = isMobile ? 0.0045 : 0.003;

  return {
    cellSize: 0.75 * mobileScale,
    zoomLevel: isMobile ? 1.2 : 1.25,
    lerpFactor: isMobile ? 0.09 : 0.075,
    dragSensitivity,
...
export const config = getGalleryConfig();
...
export const fragmentShader = `
  ...
  void main() {
    ...
    vec2 distorted = screenUV * distortion;

    vec2 aspectRatio = vec2(uResolution.x / uResolution.y, 1.0);
    vec2 worldCoord = distorted * aspectRatio;
    worldCoord *= uZoom;
    worldCoord += uOffset;
    ...
    bool isCurrentCellHovered = cellId.x == mouseCellId.x && cellId.y == mouseCellId.y && uMousePos.x >= 0.0;
    ...
    float hoverBorderDistance = min(min(cellUV.x, 1.0 - cellUV.x), min(cellUV.y, 1.0 - cellUV.y));
    float hoverBorderMask = 1.0 - smoothstep(0.04, 0.15, hoverBorderDistance);
    ...
    if (inTextArea) {
      ...
      if (textCoord.x < 0.0) textCoord.x = 0.0;
      if (textCoord.x > 1.0) textCoord.x = 1.0;
    ...
    if (isCurrentCellHovered) {
      color = mix(color, vec3(0.15, 1.0, 0.45), hoverBorderMask * 0.95);
    }
